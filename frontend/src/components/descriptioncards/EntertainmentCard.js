...
export const EntertainmentCard = () => {
  ...
  const fetchMovie = useCallback(() => {
    ...
    .then(res => res.json())
    .then((json) => {
      const getMovie = json.results[Math.floor(Math.random() * json.results.length)];
      const getMovieTwo = json.results[Math.floor(Math.random() * json.results.length)];
      setMovie(getMovie);
      ...
      <MovieContainer>
          ...
          <MovieTextContainer>
            ...
            <RatingImdb>
              ...
              <ImdbLink href={`https://www.imdb.com/title/${movie.imdb_id}`}>IMDb</ImdbLink>
            </RatingImdb>
            <MovieDescription>{movie.overview}</MovieDescription>
            <UpgradeButton onClick={() => setPremium(true)}>Upgrade to premium</UpgradeButton>
          </MovieTextContainer>
        </MovieContainer>
        <MovieContainerPremium premium={!premium}>
          <MovieImage src={`https://image.tmdb.org/t/p/w342${movieTwo.poster_path}`} />
          <MovieTextContainer>
            <TitleCircleBox>
            ...
            <RatingImdb>
              ...
              <ImdbLink href={`https://www.imdb.com/title/${movieTwo.imdb_id}`}>IMDb</ImdbLink>
            </RatingImdb>
